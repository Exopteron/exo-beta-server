...
use core::{any::{TypeId, type_name}, fmt, mem::MaybeUninit, slice};

use crate::{
    archetype::{TypeNameMap, TypeInfo},
    Archetype, Component,
};
...
impl ColumnBatchType {
    ...
    pub fn into_batch(self, size: u32) -> ColumnBatchBuilder {
        ...
        types.dedup();
        let fill = TypeNameMap::with_capacity_and_hasher(types.len(), Default::default());
...
impl ColumnBatchBuilder {
    ...
    pub fn writer<T: Component>(&mut self) -> Option<BatchWriter<'_, T>> {
        ...
        Some(BatchWriter {
            fill: self.fill.entry(type_name::<T>()).or_insert(0),
            storage: unsafe {
