...
use crate::archetype::{Archetype, TypeNameMap, TypeInfo};
...
pub struct World {
    ...
    bundle_to_archetype: TypeNameMap<u32>,
    id: u64,
}
...
impl World {
    ...
    pub fn remove<T: Bundle + 'static>(&mut self, entity: Entity) -> Result<T, ComponentError> {
        ...
        let target = match source_arch.remove_edges.get(&type_name::<T>()) {
            Some(&x) => x,
            None => {
...
struct ArchetypeSet {
    ...
    index: HashMap<Box<[&'static str]>, u32>,
    ...
    insert_edges: Vec<TypeNameMap<InsertTarget>>,
}
...
impl ArchetypeSet {
    ...
    fn get<T: Borrow<[&'static str]> + Into<Box<[&'static str]>>>(
        &mut self,
        components: T,
    ...
    fn insert(&mut self, components: Box<[&'static str]>, info: Vec<TypeInfo>) -> u32 {
