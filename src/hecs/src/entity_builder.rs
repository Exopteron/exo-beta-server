...
use crate::archetype::{TypeNameMap, TypeInfo};
...
impl EntityBuilder {
    ...
    pub fn has<T: Component>(&self) -> bool {
        self.indices.contains_key(&type_name::<T>())
    }

    /// Borrow the component of type `T`, if it exists
    pub fn get<T: Component>(&self) -> Option<&T> {
        let index = self.indices.get(&type_name::<T>())?;
    ...
    pub fn component_types(&self) -> impl Iterator<Item = &'static str> + '_ {
...
unsafe impl DynamicBundle for BuiltEntity<'_> {
    fn with_ids<T>(&self, f: impl FnOnce(&[&'static str]) -> T) -> T {
        f(&self.builder.ids)
    }
