...
pub struct Archetype {
    types: Vec<TypeInfo>,
    index: OrderedTypeNameMap<usize>,
    len: u32,
    entities: Box<[u32]>,
    ...
    pub(crate) remove_edges: TypeNameMap<u32>,
}
...
impl Archetype {
    ...
    pub(crate) fn new(types: Vec<TypeInfo>) -> Self {
        ...
        Self {
            index: OrderedTypeNameMap::new(types.iter().enumerate().map(|(i, ty)| (ty.id, i))),
            types,
            entities: Box::new([]),
            len: 0,
    ...
    pub(crate) fn get_base<T: Component>(&self, state: usize) -> NonNull<T> {
        assert_eq!(self.types[state].id, type_name::<T>());
    ...
    ///
    /// [`Entity`]: crate::Entity
    pub fn component_types(&self) -> impl ExactSizeIterator<Item = &'static str> + '_ {
    ...
    pub(crate) unsafe fn move_to(
        &mut self,
        index: u32,
        mut f: impl FnMut(*mut u8, &'static str, usize),
    ) -> Option<u32> {
        let last = self.len - 1;
        for (ty, data) in self.types.iter().zip(&*self.data) {
...
#[derive(Default)]
pub(crate) struct TypeNameHasher {
    hash: u64,
}

impl Hasher for TypeNameHasher {
...
pub(crate) type TypeNameMap<V> = HashMap<&'static str, V, BuildHasherDefault<TypeNameHasher>>;

struct OrderedTypeNameMap<V>(Box<[(&'static str, V)]>);

impl<V> OrderedTypeNameMap<V> {
    fn new(iter: impl Iterator<Item = (&'static str, V)>) -> Self {
        let mut vals = iter.collect::<Box<[_]>>();
        vals.sort_unstable_by_key(|(id, _)| *id);
        Self(vals)
    }

    fn search(&self, id: &'static str) -> Option<usize> {
        self.0.binary_search_by_key(&id, |(id, _)| *id).ok()
    }

    fn contains_key(&self, id: &'static str) -> bool {
        self.search(id).is_some()
    }

    fn get(&self, id: &'static str) -> Option<&V> {
