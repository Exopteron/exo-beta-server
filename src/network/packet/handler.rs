    ...
    match packet {
        ...
        ClientPacket::PlayerPositionPacket(packet) => {
            ...
            if !bad_move {
                ...
                player.write_packet(ServerPacket::PlayerPositionAndLook {
                    x: pos.x,
                    stance: pos.stance,
                    y: pos.y + 0.1,
                    z: pos.z,
            ...
            match packet.status {
                0 => {
                    ...
                    if let Some(item) = registry.get_item(orig_type as i16) {
                        if let Some(item) = item.get_item().as_block() {
                            if !item.insta_break() {
                                player.write(ServerPacket::BlockChange {
                                    x: packet.x,
                                    y: packet.y + 0,
                    ...
                    if let Some(item) = registry.get_item(orig_type as i16) {
                        if let Some(block) = item.get_item().as_block() {
                            ...
                            if let Some(drop) = block.on_break(
                                game,
                                packet.clone(),
                                player,
                                tool,
        ...
        if packet.item_id != -1 {
                ...
                if packet.right_click == 0 {
                    if !packet.shift {
                        ...
                        *player.get_current_cursored_item_mut() = Some(recv_item);
                        ...
                        if let WClick::Inventory(_) = inventory {
                            if window_type == 1 {
            ...
            if packet.slot > 0 {
                ...
                /*                     for i in 1..4 {
                    ...
                    grid[0][1] = if let Some(a) = inv.items.get(&2) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
                    ...
                    grid[0][2] = if let Some(a) = inv.items.get(&3) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
                    ...
                    grid[1][0] = if let Some(a) = inv.items.get(&4) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
                    ...
                    grid[1][2] = if let Some(a) = inv.items.get(&6) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
                    ...
                    grid[2][0] = if let Some(a) = inv.items.get(&7) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
                    ...
                    grid[2][1] = if let Some(a) = inv.items.get(&8) {
                        let a = a.clone();
                        if a.id == 0 {
                            None
                        } else {
