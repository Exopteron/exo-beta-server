...
impl Item for FlintAndSteelItem {
    ...
    fn on_use(&self, game: &mut Game, server: &mut crate::server::Server, item_slot: usize, user: Entity, target: Option<crate::item::item::BlockUseTarget>) -> SysResult {
        if let Some(target) = target {
            ...
            if game.block_id_at(pos, target.world) == 0 {
...
impl Block for FireBlock {
    ...
    fn added(&self, world: i32, game: &mut Game, server: &mut crate::server::Server, position: BlockPosition, state: BlockState) {
        if game.block_id_at(position.offset(0, -1, 0), world) == 49 && PortalBlock::try_create_portal(world, game, position) {
            return;
        }
...
impl FireBlock {
    pub fn chance_of_neighbors_ef(world: i32, game: &mut Game, pos: BlockPosition) -> i32 {
        let mut l = 0;
        if !game.block_id_at(pos, world) == 0 {
            return 0;
        } else {
