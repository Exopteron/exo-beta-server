...
impl Show for Grid {
    fn print(&self) {
        log::info!("{} {} {}", self[0][0].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[0][1].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[0][2].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()));
        log::info!("{} {} {}", self[1][0].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[1][1].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[1][2].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()));
        log::info!("{} {} {}", self[2][0].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[2][1].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()), self[2][2].as_ref().map(|s| s.id().to_string()).unwrap_or_else(|| "Air".to_string()));
    }
}
...
impl ShapedRecipe {
    pub fn new(input: Grid, output: ItemStack) -> Self {
        Self {
            input,
            output
        }
...
impl ShapelessRecipe {
    pub fn new_from_vec(input: Vec<ItemStack>, output: ItemStack) -> Self {
        let mut array_vec = ArrayVec::new();
        for val in input {
            array_vec.push(val);
        }
        Self::new(array_vec, output)
    }
...
pub struct Solver {
    ...
    shapeless: AHashMap<ArrayVec<[ItemStack; TABLE_SIZE]>, ItemStack>,
}
...
impl Solver {
    ...
    pub fn solve(&self, input: &mut Grid) -> Option<ItemStack> {
        ...
        for row in input.iter_mut() {
            for item in row.iter_mut().flatten() {
                item.set_count(1);
                item.set_damage(0);
            }
        }
        ...
        if let Some(output) = self.shaped.get(input).cloned() {
            ...
            self.shapeless.get(&inputs).cloned()
        }
    }
    ...
    pub fn register_shaped(&mut self, shaped: ShapedRecipe) {
    ...
    pub fn register_shapeless(&mut self, shapeless: ShapelessRecipe) {
...
pub fn translate(grid: &mut Grid, x: usize, y: usize) {
    ...
    grid.moveslice(x..TABLE_WIDTH, 0);
    // Upward
    for column in grid.iter_mut() {
        column.moveslice(y..TABLE_WIDTH, 0);
    }
...
mod tests {
    ...
    fn shaped() {
        ...
        solver.register(Recipe::Shaped(ShapedRecipe {
            input: grid,
            output: ItemStack::new(5, 0, 1),
        }));
        let mut grid = Grid::default();
        grid[0][0] = Some(ItemStack::new(1, 0, 1));
    ...
    fn shapeless() {
        ...
        let shapeless = ShapelessRecipe {
            input: arrvec,
            output: ItemStack::new(3, 0, 1),
        };
        let mut solver = Solver::new();
